    ...
    <div class="theme-view">
        <div class="theme-view__header">
            ...
            <p class="theme-view__intro">
                Customize this site's colors. Anything left unset uses the default shown in its field —
                changes here only affect this site's own pages, not the admin dashboard.
            </p>
        </div>
        ...
        <div v-else class="theme-editor">
            <form class="theme-form" novalidate @submit.prevent="handleSave">
                <div v-for="group in fieldGroups" :key="group.name" class="theme-form__group">
                    <p class="theme-form__group-title">{{ group.name }}</p>

                    <div v-for="field in group.fields" :key="field.key" class="color-field">
                        <label class="color-field__label" :for="`theme-${field.key}`">{{ field.label }}</label>
